...
#Thông tin cấu hình phiên bản cũ dành cho thiết bị di động Thích ứng với phiên bản nhỏ hơn 1.1.0
...
check_system() {
    if [[ "${ID}" == "centos" && ${VERSION_ID} -ge 7 ]]; then
        echo -e "${OK} ${GreenBG} Phiên bản hiện tại là Centos ${VERSION_ID} ${VERSION} ${Font}"
        INS="yum"
    elif [[ "${ID}" == "debian" && ${VERSION_ID} -ge 8 ]]; then
        echo -e "${OK} ${GreenBG} Phiên bản hiện tại là Debian ${VERSION_ID} ${VERSION} ${Font}"
        INS="apt"
        $INS update
        ## 添加 Nginx apt源
    elif [[ "${ID}" == "ubuntu" && $(echo "${VERSION_ID}" | cut -d '.' -f1) -ge 16 ]]; then
        echo -e "${OK} ${GreenBG} Phiên bản hiện tại là Ubuntu ${VERSION_ID} ${UBUNTU_CODENAME} ${Font}"
        INS="apt"
        ...
        echo -e "${Error} ${RedBG} Phiên bản hiện tại là ${ID} ${VERSION_ID} 不在支持的系统列表内，Cài đặt中断 ${Font}"
        exit 1
    fi
...
is_root() {
    if [ 0 == $UID ]; then
        echo -e "${OK} ${GreenBG} Người dùng hiện tại là người dùng root, hãy nhập quá trình Install ${Font}"
        sleep 3
    else
...
chrony_install() {
    ...
    judge "Cài đặt chrony Đồng bộ thời gian "
...
dependency_install() {
    ...
    judge "crontab Cấu hình tự khởi động "
    ...
    if [[ "${ID}" == "centos" ]]; then
        ...
        #       judge "rng-tools khởi động"
        systemctl start haveged && systemctl enable haveged
        #       judge "haveged khởi động"
    else
...
port_alterid_set() {
    if [[ "on" != "$old_config_status" ]]; then
        read -rp "Vui lòng nhập cổng （default:443）:" port
        [[ -z ${port} ]] && port="443"
        read -rp "Vui lòng nhập số alterID（default:2 Chỉ những số được phép）:" alterID
        [[ -z ${alterID} ]] && alterID="2"
    fi
}
modify_path() {
...
v2ray_install() {
    ...
    if [[ -f v2ray.sh ]]; then
        ...
        echo -e "${Error} ${RedBG} V2ray Cài đặt文件Tải xuống失败，请检查Tải xuống地址是否可用 ${Font}"
        exit 4
    fi
    # 清除临时文件
    rm -rf /root/v2ray
...
nginx_exist_check() {
    if [[ -f "/etc/nginx/sbin/nginx" ]]; then
        echo -e "${OK} ${GreenBG} Nginx đã tồn tại, bỏ qua quá trình cài ${Font}"
        sleep 2
    elif [[ -d "/usr/local/nginx/" ]]; then
        echo -e "${OK} ${GreenBG}Nginx của gói khác Cài đặt được phát hiện, nếu bạn tiếp tục Cài đặt sẽ gây ra xung đột, vui lòng xử lý sau khi Cài đặt${Font}"
        exit 1
    else
        nginx_install
    fi
...
nginx_install() {
    ...
    judge "Nginx Tải xuống"
    wget -nc --no-check-certificate https://www.openssl.org/source/openssl-${openssl_version}.tar.gz -P ${nginx_openssl_src}
    judge "openssl Tải xuống"
    wget -nc --no-check-certificate https://github.com/jemalloc/jemalloc/releases/download/${jemalloc_version}/jemalloc-${jemalloc_version}.tar.bz2 -P ${nginx_openssl_src}
    judge "jemalloc Tải xuống"
    ...
    echo -e "${OK} ${GreenBG} 即将开始编译Cài đặt jemalloc ${Font}"
    sleep 2
    ...
    judge "jemalloc 编译Cài đặt"
    echo '/usr/local/lib' >/etc/ld.so.conf.d/local.conf
    ldconfig

    echo -e "${OK} ${GreenBG} 即将开始编译Cài đặt Nginx, 过程稍久，请耐心等待 ${Font}"
    sleep 4
...
ssl_install() {
    ...
    judge "Cài đặt SSL 证书生成脚本依赖"

    curl https://get.acme.sh | sh
    judge "Cài đặt SSL chứng chỉ"
}
domain_check() {
    read -rp "Nhập tên miền(eg:www.wulabing.com):" domain
    domain_ip=$(ping "${domain}" -c 1 | sed '1{s/[^(]*(//;s/).*//;q}')
    echo -e "${OK} ${GreenBG} Đang lấy thông tin ip mạng public, vui lòng kiên nhẫn chờ ${Font}"
    local_ip=$(curl https://api-ipv4.ip.sb/ip)
    echo -e "Tên miền dns phân giải IP：${domain_ip}"
    echo -e "Local IP: ${local_ip}"
    sleep 2
    if [[ $(echo "${local_ip}" | tr '.' '+' | bc) -eq $(echo "${domain_ip}" | tr '.' '+' | bc) ]]; then
        echo -e "${OK} ${GreenBG} Tên miền dns phân giải IP khớp với Local IP ${Font}"
        sleep 2
    else
        echo -e "${Error} ${RedBG} Hãy đảm bảo rằng bản ghi A chính xác được thêm vào tên miền, nếu không nó sẽ không hoạt động bình thường V2ray ${Font}"
        echo -e "${Error} ${RedBG} Tên miền dns phân giải IP không Local IP. Có tiếp tục Cài đặt？（y/n）${Font}" && read -r install
        case $install in
        [yY][eE][sS] | [yY])
            echo -e "${GreenBG} Tiếp tục Cài đặt ${Font}"
            sleep 2
...
port_exist_check() {
    if [[ 0 -eq $(lsof -i:"$1" | grep -i -c "listen") ]]; then
        echo -e "${OK} ${GreenBG} $1 Port trống ${Font}"
        sleep 1
    else
        echo -e "${Error} ${RedBG} Phát hiện $1 port đã sử dụng，Sau đây là thông tin port $1 sử dụng ${Font}"
        lsof -i:"$1"
...
basic_information() {
    {
        ...
        echo -e "${Red} Tên id（UUID）：${Font} $(info_extraction '\"id\"')"
        echo -e "${Red} Tên bổ sung id（alterId）：${Font} $(info_extraction '\"aid\"')"
...
tls_type() {
    if [[ -f "/etc/nginx/sbin/nginx" ]] && [[ -f "$nginx_conf" ]] && [[ "$shell_mode" == "ws" ]]; then
        echo "Vui lòng chọn phiên bản TLS được hỗ trợ (mặc định: 3):"
        echo "Xin lưu ý rằng nếu bạn sử dụng Quantaumlt X / router / Shadowrocket / V2ray core cũ thấp hơn 4.18.1, vui lòng chọn chế độ tương thích"
        ...
        systemctl restart nginx
        judge "Nginx Khởi động lại"
    else
        echo -e "${Error} ${RedBG} Nginx Hoặc tệp cấu hình không tồn tại hoặc phiên bản hiện tại của Cài đặt là h2, vui lòng thực hiện đúng kịch bản Cài đặt${Font}"
    fi
}
show_access_log() {
...
uninstall_all() {
    ...
    if [[ -d $nginx_dir ]]; then
        echo -e "${OK} ${Green} Có gỡ cài đặt hay không Nginx [Y/N]? ${Font}"
...
menu() {
    update_sh
    echo -e "\t V2ray Tệp lệnh quản lý cài đặt ${Red}[${shell_version}]${Font}"
    echo -e "\t---authored by wulabing---"
    echo -e "\thttps://github.com/wulabing\n"
    echo -e "Phiên bản hiện tại:${shell_mode}\n"

    echo -e "—————————————— Hướng dẫn cài đặt ——————————————"""
    echo -e "${Green}0.${Font}  Cập nhật"
    echo -e "${Green}1.${Font}  Cài đặt V2Ray (Nginx+ws+tls)"
    echo -e "${Green}2.${Font}  Cài đặt V2Ray (http/2)"
    echo -e "${Green}3.${Font}  Cập nhật V2Ray core"
    ...
    echo -e "${Green}7.${Font}  Thay đổi phiên bản TLS(Chỉ ws + tls là hợp lệ)"
    echo -e "${Green}18.${Font}  Đổi đường dẫn"
    echo -e "—————————————— Kiểm tra cấu hình ——————————————"
    echo -e "${Green}8.${Font}  Kiểm tra lịch sử truy cập"
    echo -e "${Green}9.${Font}  Kiểm tra lịch sử lỗi"
    echo -e "${Green}10.${Font} Kiểm tra cấu hình V2Ray"
    echo -e "—————————————— Khác ——————————————"
    echo -e "${Green}11.${Font} Cài đặt 4 trong 1 bbr lệnh cài đặt"
    echo -e "${Green}12.${Font} Cài đặt MTproxy(Hỗ trợ làm mờ TLS)"
    echo -e "${Green}13.${Font} Cập nhật chứng chỉ"
    echo -e "${Green}14.${Font} Gỡ V2Ray"
    echo -e "${Green}15.${Font} Cập nhật chứng chỉ crontab tác vụ đã lên lịch"
    echo -e "${Green}16.${Font} Tệp kế thừa chứng chỉ trống"
    ...
    5)
        ...
        ;;
    6)
        read -rp "Vui lòng nhập cổng:" port
        if grep -q "ws" $v2ray_qr_config_file; then
            modify_nginx_port
        elif grep -q "h2" $v2ray_qr_config_file; then
            modify_inbound_port
